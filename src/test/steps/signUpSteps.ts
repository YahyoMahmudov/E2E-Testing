...
let randomNum;
Then('User clicks {string} on {string}',
...
Then('User fills {string} as {string} on {string}',
  async function  (placeholder, input, page) {
...
Then('User enters {string} as {string} on {string}', async function (email, input, page) {
  ...
  const toCamelCase = basePage.wrapper.toCamelCase;
  const [pageName, placeholderName] = toCamelCase(page, email);
